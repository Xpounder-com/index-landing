...
- Set `VITE_DEMO_URL` to the product login handoff URL, or confirm the built-in Index product handoff is still correct.
...
- Verify `Try live demo` captures a work email, reveals demo access, stores local demo access state, and opens the configured product login URL.
